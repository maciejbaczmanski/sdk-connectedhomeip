...
static constexpr uint32_t kMaxICDClientKeys = CHIP_CONFIG_MAX_ICD_CLIENTS;
...
static_assert(to_underlying(KeyIdBase::Minimum) >= CHIP_CONFIG_CRYPTO_PSA_KEY_ID_BASE &&
              to_underlying(KeyIdBase::Maximum) <= CHIP_CONFIG_CRYPTO_PSA_KEY_ID_END, "PSA key ID base out of allowed range");
